...
\centering
\begin{tikzpicture}[table/.style={matrix of nodes, nodes in empty cells, nodes={rectangle, anchor=center, minimum height=1.25em, 
      font=\sffamily,text height=1.5ex, text depth=.25ex, align=center, draw}}]

\matrix(expedition) [table, label=above:Expedition, text width=7em, column 1/.style={nodes={text width=1.5em}}] {id & name\\0 & Expedition 1\\1 & Expedition 2\\};
...
\matrix(group) [table, label=above:Group, left=.5em of expedition, text width=5em, column 1/.style={nodes={text width=1.5em}}] {id & name\\0 & Group 1\\};
...
\draw(group-2-1.south)--++(-.5em,-2.75em)-|(belongsto-1-1.north);
\draw (group-2-1.south)--++(.5em,-2.75em)-|(interestedin-1-2.north);
\draw (expedition-3-1.south)--++(-.5em,-3em)-|(belongsto-1-2.north);
\draw (expedition-3-1.south)--++(.5em,-3em)-|(joinsexpedition-1-2.north);
\draw (member-3-1.south)--++(-.5em,-4em)-|(interestedin-1-1.north);
\draw (member-3-1.south)--++(.5em,-4em)-|(joinsexpedition-1-1.north);
\end{tikzpicture}
\caption{}
...
\centering
\begin{tikzpicture}[>=stealth', shorten >=1pt, node distance=2.5em, on grid, every text node part/.style={align=center}, every edge/.style={->}]
	\node[state] (m0) {Member\\ \small{id: 0\\ name: Kim}};
 	\node[state] (m1) [right =of m0] {Member\\ \small{id: 1\\ name: Alex}};
 	\node[state] (g0) [below left =of m0] {Group\\ \small{id: 0\\ name: Group 1}};  
 	\node[state] (e0) [below right =of m0] {Expedition\\  \small{id: 0\\ name: Expedition 1}};
 	\node[state] (e1) [right =of e0] {Expedition\\  \small{id: 1\\ name: Expedition 2}};
	\path (m0) edge node {INTERESTED\_IN} (g0)
	(m1) edge node {INTERESTED\_IN} (g0)
	(e0) edge node {BELONGS\_TO} (g0)
	(e1) edge node {BELONGS\_TO} (g0)
	(m0) edge node {JOINS\_EXPEDITION} (e1)
	(m1) edge node {JOINS\_EXPEDITION} (e0);
\end{tikzpicture}
\caption{}
