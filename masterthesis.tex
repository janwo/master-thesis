...
\centering
\begin{lstlisting}
// Retrieve the nearest excursions within the distance of 0.5 kilometers to u.
MATCH (u:User {id: $user.id})
WITH u CALL spatial.closest("Expedition", $user.location, 0.5) YIELD node AS e 
WITH e, u 

// Keep only the excursions that belong to a joined interest group of u and are dated within the next seven days.
...
// Return the ten topmost recent excursions.
ORDER BY e.date 
LIMIT 10
RETURN e
\end{lstlisting}
\caption[Algorithm to recommend excursions]{The entire algorithm to recommend excursions to a particular user.}
\label{recommendationalogirthmexcursions}
\end{figure}
