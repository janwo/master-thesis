...
%Unsere Gesellschaft befindet sich durch das manifestierende Paradigma ständiger Erreichbarkeit in einer erneuten Auseinandersetzung mit dem Thema der Zeit und Freizeit. Sorgte früher die Industrailisung für eine nie dagewesene rationalisierung der Zeit, sorgen heute Echtzeitnachrichten und die erwatungen an immer effizienteren prozessen für ein gefühl der zeitknappheit. Studien zeigen den Anstieg an solitären Aktivitäten. Zeit soll gespart werden. Soziale medien verbinden uns auf virtuellem weg. Die qualität der kommunikation transformiert sich verstärkt in quantitatives. In der vorliegende thesis wird eine guideline vorgestellt und anhand dessen eine application implementiert, die es mitgliedern ermöglichen soll, die Freizeitgestatlung mit möglichst geringem einsatz von echtzeitkommunikation zu ermöglichen und dabei trotzdem aus den heutigen technologischen Möglichkeiten zu schöpfen. Mit Hilfe von Empfehlungsalgorithmen und der kollaborativen Organisation von Freizeit, soll dies gelingen. In einer anschließenden case study zeigt sich, dass die Idee begrüßt wird und die Empfehlungsalgorithmen den Mitgliedern signifikante Vorschläge für die Freizeit machen. Möglicherweise wäre diese Form ein Ansatz, die Kommunikation zu entschleunigen und das Gefühl der Zeitknappheit entgegenzuwirken.
...
\end{axis}
\end{tikzpicture}
\caption[Trending and abondoning leisure activities]{Top-five trending and abandoning leisure activities done at least once a week in a 5-year comparison by those polled. \citep{freizeitmonitor2016}}
...
\label{activitymap}
\end{subfigure}%
\caption[Organization of activities]{The activity panel (\subref{activity}) comes with a map of the meeting point (\subref{activitymap}), invitation capabilities (\subref{activityattendees}) and a discussion board (\subref{activitydiscussion}) to let members plan their activity in detail.}
\end{figure}
...
\end{subfigure}%
\caption[Profile management and notifications]{Every member has a public profile (\subref{user}), which can be configured via the settings page (\subref{settings}). Every member receives personal notifications (\subref{notifications}) as well as an overview of all joined activities (\subref{useractivities}).}
\end{figure}
...
\end{subfigure}%
\caption[Utilization of the recommendation algorithm]{Utilization of the recommendation algorithm. (\subref{utilizealgorithm0}) (\subref{utilizealgorithm1}) }
...
\rotatebox[origin=c]{90}{Facebook} & Facebook provides a less specific service (3 people, 50\%) and its members often know each other in person (2 people, 33\%). & More members has been registered yet (3 people, 50\%). & More difficult to keep overview (3 people, 50\%) and more advertisement (2 people, 33\%). \\
\rotatebox[origin=c]{90}{Event Calendar} & Event calendar provides pre-scheduled activities that cannot be influenced any longer (4 people, 67\%). & Calendrical layout (3 people, 50\%) and a large choice of activities (2 people, 33\%). & Recommendation of activities is not personalized (2 people, 33\%) and likely no ability to contribute (2 people, 33\%). \\ 
\rotatebox[origin=c]{90}{Instant Messenger} & The main focus of instant messengers is the communication between individuals (4 people, 67\%). & More instant feedback (5 people, 83\%). & More difficult to keep overview (2 people, 33\%) and likely not made for activity planning (2 people, 33\%). \\ 
\rotatebox[origin=c]{90}{Friends \& Acquaintances} & Level of intimacy (3 people, 50\%) and directness of feedback (2 people, 33\%). & Planning can start more spontaneously (3 people, 50\%), the communication feels intimate (2 people, 33\%) and meetings are face-to-face (2 people, 33\%). & The number of recommended activities is smaller (3 people, 50\%), a greater effort is needed while planning in big groups (2 people, 33\%) and it is more difficult to check or report attendance status (2 people, 33\%). \\ \hline
\end{tabularx}
...
METHOD:
This respects the guideline and the reliability of recommendations made by the application. 
Not patronized

%Ergebnisse der Studie wiederholen und in den Kontext stellen
%sobald ohysisch, ist es nicht einfach ein klick, sondern es beduetet arbeit

\chapter{Conclusion}


%Thema, wie in der Einführung, aufgreifen
%Ausblick formulieren
%Implementation von Bucketlisten?
