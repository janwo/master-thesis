...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{searchrudel.png}
\caption{}
\label{searchrudel}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{createrudel0.png}
\caption{}
\label{createrudel0}
\end{subfigure}
...
\label{rudel}
\end{subfigure}
\caption[Organization of interest groups]{The panel of an interest group (\subref{rudel}) reveals all important information and is accessible via the search interface (\subref{searchrudel}). Interests groups are created in two steps: (\subref{createrudel0}) specify a name for the interest group and (\subref{createrudel1}) specify an icon for the interest group.}
\end{figure}
...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{createexpedition2.png}
\caption{}
\label{createexpedition2}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{createexpedition3.png}
\caption{}
\label{createexpedition3}
\end{subfigure}
\caption[Creation of activities]{Creation of activities in four steps: (\subref{createexpedition0}) Adjust the activity name and access control, (\subref{createexpedition1}) redefine the activity icon, (\subref{createexpedition2}) set the meeting point and (\subref{createexpedition3}) specify date and time of the activity.}
\end{figure}

\begin{figure}
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{expedition.png}
\caption{}
\label{expedition}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{expeditionattendees.png}
\caption{}
\label{expeditionattendees}
\end{subfigure}
...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{expeditiondiscussion.png}
\caption{}
\label{expeditiondiscussion}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{expeditionmap.png}
\caption{}
\label{expeditionmap}
\end{subfigure}
\caption[Organisation of activities]{The activity panel (\subref{expedition}) comes with a map of the meeting point (\subref{expeditionmap}), invitation capabilities (\subref{expeditionattendees}) and a discussion board (\subref{expeditiondiscussion}) to let members plan their activity in detail.}
\end{figure}
...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{userexpeditions.png}
\caption{}
\label{userexpeditions}
\end{subfigure}
\caption[Profile Management and Notifications]{Every member has a public profile (\subref{user}), which can be configured via the settings page (\subref{settings}). Every member receives personal notifications (\subref{notifications}) as well as an overview of all joined activities (\subref{userexpeditions}).}
\end{figure}
...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{exploreexpeditions.png}
\caption{}
\label{exploreexpeditions}
\end{subfigure}
...
\label{exploreusers}
\end{subfigure}
\caption[Explorative Components]{Explorative components either display lists of interest groups (\subref{explorerudel}), activities (\subref{exploreexpeditions}) or users (\subref{exploreusers}). All lists are either sorted by popularity, recommendation quality or recency.}
\label{explore}
\end{figure}
...
\matrix(expedition) [table, label = above:Expedition, text width = 3em, column 1/.style = {nodes = {text width = 1.5em}}] {id & name\\0 & Exp1\\1 & Exp2\\};

\matrix(member) [table, label = above:Member, text width = 3em, right = .5em of expedition, column 1/.style = {nodes = {text width = 1.5em}}] {id & name\\0 & Kim \\1 & Alex\\};

\matrix(group) [table, label = above:Group, left = .5em of expedition, text width = 3em, column 1/.style = {nodes = {text width = 1.5em}}] {id & name\\0 & Grp1\\};
    
\matrix(interestedin) [table, label = below:INTERESTED\_IN, below = 5em of expedition, text width = 4.5em, column 1/.style = {nodes = {text width = 3em}}] {group & member\\0 & 0\\0 & 1\\};
    
\matrix(belongsto) [table, label = below:BELONGS\_TO, left = .5em of interestedin, text width = 5.5em, column 1/.style = {nodes = {text width = 3em}}] {group & expedition\\0 & 0\\0 & 1\\};

\matrix(joinsexpedition) [table, label = below:JOINS\_EXPEDITION, right = .5em of interestedin, text width = 5.5em] {member & expedition\\0 & 1\\1 & 0\\};

\draw[thick] (group-2-1.south)--++(-.5em,-2.75em)-|(belongsto-1-1.north);
\draw[thick] (group-2-1.south)--++(.5em,-2.75em)-|(interestedin-1-2.north);
\draw[thick] (expedition-3-1.south)--++(-.5em,-3em)-|(belongsto-1-2.north);
\draw[thick] (expedition-3-1.south)--++(.5em,-3em)-|(joinsexpedition-1-2.north);
\draw[thick] (member-3-1.south)--++(-.5em,-4em)-|(interestedin-1-1.north);
\draw[thick] (member-3-1.south)--++(.5em,-4em)-|(joinsexpedition-1-1.north);
\end{tikzpicture}
\caption{}
...
\begin{tikzpicture}[ >= stealth', shorten >= 1pt, node distance = 15em, on grid, every text node part/.style = {align = center}]
	...
	\node[state] (m1) [above right = of g0] {Member\\ \textit\small{id: 1}\\ \textit\small{name: Alex}};
 	\node[state] (e0) [below right = of g0] {Expedition\\ \textit\small{id: 0}\\ \textit\small{name: Exp1}};
 	\node[state] (e1) [below left= of g0] {Expedition\\ \textit\small{id: 1}\\ \textit\small{name: Exp2}};
	...
	\path[->, thick] (e0) edge node[fill = white, inner sep = 3pt] {BELONGS\_TO} (g0);
	\path[->, thick] (e1) edge node[fill = white, inner sep = 3pt] {BELONGS\_TO} (g0);
	\path[->, thick] (m0) edge node[fill = white, inner sep = 3pt] {JOINS\_EXPEDITION} (e1);
	\path[->, thick] (m1) edge node[fill = white, inner sep = 3pt] {JOINS\_EXPEDITION} (e0);
\end{tikzpicture}
\caption{}
...
\chapter{Method}
This chapter describes used methods to review the proposals of actions, which are mentioned in the previous chapter, and its consequences. The primary method is a case study involving interviews with interested members on the basis of a questionnaire. A prior utilization of the recommendation algorithm ensures the correct processing of the algorithm. Due to the diverse nature of the field, the case study cannot give a complete picture of all the effects involved as it certainly requires an interdisciplinary evaluation with more resources. Nonetheless, it should be capable to generate enough insights to give a general impression of the members opinion. This respects the proposals of actions and the reliability of recommendations made by the application.
...
The System Usability Scale was developed in 1986 by John Brooke and is made up of ten questions to evaluate a given system \citep[p.189-194]{brooke1996}. The questionnaire is a common tool to quickly check the usability of a system. Good usability is a prerequisite for saving time as cumbersome functionalities increase the total interaction time unnecessarily. For this reason, a system usability scale will be developed. Every participant is asked to complete a system usability scale during the interview.
