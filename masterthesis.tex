...
% Dafür jedoch die möglichkeit von interaktionen und personalisierung. Dies wurde auch gleichzeitig als häufigster Hauptunterschied gesehen.
...
% ABER: Auch wenn viele sich untereinander kannten, wurde von jedem interviewee durchschnittlich eine aktivität besucht, die es ohne die app nicht gegeben hätte => toll! Beeinflusst! Ähnliches Wurde auch nochmal von einer person unter vorteieln genannt, dass sie neues entdeckt hat.
...
\chapter{Conclusion}
The thesis skizzierte die entwicklung von der erfindung der uhr bis hin zur vernetzten welt auf und betrachtete dabei the influence of our perception of leisure time. Sie zeigte auf, wie media changed the way we organize our free time. Wir sind es gewohnt, eine große fülle an mölgichen freizeit aktivitäten und empfehlungen zu erhalten und finden dies innerhalb verschiedener kanäle, vor allem in social networks. Mit diesem Wissendurst gelockt, versuchen die sozialen netzwerke uns an ihre plattform zu binden und sich weiterhin als das medium zwischen den menschen zu etablieren. Die thesis machte auf Studien aufmerksam, die einen trend zu weniger gemeinsamen aktiivitäten feststellen. Um dem entgegenzuwirken, wurden mehrere statements forumluiert und in einer guideline zusammengefasst. Der daraus entstammende prototyp hielt den grad an selbstdarstellung, kommunikation und informationen ohne bezug auf die freizeitgestaltung gering. Darüber hinaus wurden maschinelle empfehlungen zu möglichen Aktivitöten gemacht und dessen Mitgliedern personalisert vorgestellt. 
The thesis will call attention to the constant change of one's free time under decades of digital influence. It will investigate to what extend the ubiquitous accessibility of everyones attention certainly played its part to this development and proposes a possible solution to invert the trend. A puristic application that does not focus on chat components, self-portraits and its time-consuming relatives. The aim is to conceptualize and implement an application that helps to organize one's free time with a small timestamp online. In contrary to existing solutions like Facebook, its constant motivation is to make people meet each other in person again rather than digitally. The thesis will end with a case study of the application to validate whether it is a rather drastic and humorously ironic idea to come up with another digital application or a step forward to one's organization free time with more social activities.

%Thema, wie in der Einführung, aufgreifen
%Ausblick formulieren, future reasearch, auch im bezug auf interdisziplinärität, um tiefer in die bewegründe für das gefühl vpn pressed time einzutauchen. Eine weiterentwickung der app für private Veranstaltungen.
