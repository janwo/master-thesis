...
\centering
\begin{tikzpicture}[table/.style={matrix of nodes, nodes in empty cells, column sep=-\pgflinewidth, row sep=-\pgflinewidth, nodes={draw,anchor=center,minimum width=2cm,minimum height=2cm}, row 1/.style={nodes={minimum height=1cm}}}]

\node [relation=2, label=above:Member] (member) {id \nodepart{two} name};

\node [relation=2, label=above:Expedition, right=1cm of member] (expedition) {id%
\nodepart{two} name};

\node [relation=2, label=above:InterestGroup, left=1cm of member] (interestgroup) {id%
\nodepart{two} name };
    
\node [relation=2, label=above:INTERESTED\_IN, below=1.25cm of member] (interestedin) {member%
\nodepart{two} interest group};
    
\node [relation=2, label=above:BELONGS\_TO, left=1cm of interestedin] (belongsto) {interest group%
\nodepart{two} expedition};

\node [relation=2, label=above:JOINS\_EXPEDITION, right=1cm of interestedin] (joinsexpedition) {member%
\nodepart{two} expedition};

\draw(interestgroup.one south)--++(0,-.5)-|(belongsto.two south);
%\draw (Professor-2-3.south)--++(0,-.75)-|(Course-1-1.north);
%\draw (Course-2-1.south)--++(0,-.5)-|(Student-1-2.south);
\end{tikzpicture}
\caption{}
